  ...
  function render(data, canvas) {
    ...
    for (color in colorList) {
      if (colorList.hasOwnProperty(color)) {
        _util.addGradient(
          g1Chart.selectAll('svg')[0].parentNode,
          color,
    ...
    function getColor(nameOrindex) {
      ...
      return addedGradient.indexOf(name) > -1
        ? 'url(#grad-' + name + ')'
        : colors(tags.indexOf(name));
    }
    ...
    function drawChordDiagram() {
      var chord = d3.layout.chord().padding(0.025).matrix(matrix);
      ...
      var chordChart = g1Chart
        .append('g')
        .attr(
          'transform',
          'translate( ' + (canvasWidth - canvasHeight) + ', 0)'
        );
