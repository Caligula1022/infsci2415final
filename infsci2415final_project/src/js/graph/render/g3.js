...
define(['d3', 'utils/utils'], function (ignore, _util) {
  ...
  function render(data, canvas) {
      ...
      .forEach(function (tagName) {
        ...
        Object.keys(oneRowData).reduce(function getGrowth(a, b, i) {
          ...
          if (oneRowData[a][0] === 0) {
            ...
            oneRowData[b].push(
              ((oneRowData[b][0] - oneRowData[a][0]) / oneRowData[a][0]) * 100
            );
          }
        ...
        allCounts.push(
          oneRowData[
            Object.keys(oneRowData)[Object.keys(oneRowData).length - 1]
          ][1]
        );
    ...
    Chart.attr('viewBox', '0 0 ' + canvasWidth + ' ' + canvasHeight).attr(
      'preserveAspectRatio',
      'xMidYMid'
    );
    ...
    Object.keys(colorList).forEach(function (tag) {
      _util.addGradient(
        Chart.selectAll('svg')[0].parentNode,
        tag,
        colorList[tag].split(',')
      );
    });
    ...
    function getColor(nameOrindex) {
      var name =
        typeof nameOrindex === 'string' ? nameOrindex : tags[nameOrindex];
      return Object.keys(colorList).indexOf(name) > -1
        ? 'url(#grad-' + name + ')'
        : getColorScale(nameOrindex);
    }
    ...
    //Bg bottom
    var bottomPanel = Chart.append('g').attr(
      'transform',
      ...
      .on('mouseover', function (data) {
        d3.select(this)
          ...
          .text(function () {
            //
            return (
              data.name + ' ' + data.countData[monthData[currentMonth || 60]][0]
            );
          });
      })
    ...
    bottomPanel
      ...
      .attr('width', 32)
      .attr('height', 32)
      .attr('xlink:href', '../theme/images/arrow_left.png')
      .on('click', function () {
        currentMonth--;
        moveTag();
      });
    ...
    bottomPanel
      ...
      .attr('width', 32)
      .attr('height', 32)
      .attr('xlink:href', '../theme/images/arrow_right.png')
      .on('click', function () {
        currentMonth++;
        moveTag();
      });

    var startStop = bottomPanel
      .append('g')
    ...
    function moveTag() {
      ...
      monthLabel.text(
        new Date(+month).getMonth() + 1 + ' / ' + new Date(+month).getFullYear()
      );
